...
def TabularDisplay(a_students: dict[str, dict[str, str]]) -> None:
    ...
    """
    print(f"{"Name":<15} {" ".join([f"{subject_name:<6}" for subject_name in list(a_students.values())[0].keys()])}")
    for student_name, student_subjects in a_students.items():
        print(f"{student_name:<15} {" ".join([f"{subject_grade:<6}" for subject_grade in student_subjects.values()])}")
...
def TabularDisplayGenerator(a_students: dict[str, dict[str, str]]) -> None:
    ...
    """
    print(f"{"Name":<15} {" ".join([f"{subject_name:<6}" for subject_name in list(a_students.values())[0].keys()])}")
    print("\n".join([f"{student_name:<15} {" ".join([f"{subject_grade:<6}" for subject_grade in student_subjects.values()])}" for student_name, student_subjects in a_students.items()]))